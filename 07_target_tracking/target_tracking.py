...
起飞后悬停并搜索 H 标；识别成功后向画面中心移动，对准并保持约 1 秒后请求降落。
速度与降落请求仅发给 OFFBOARD 管理器，不得直接向 MAVROS 设定点发布。

摄像头朝下（或将 H 标正对镜头）时：
  H 偏右 → 机体右移；H 偏下 → 机体后移；框偏小 → 下降，偏大 → 上升。
  画面标注相对 H 的前/后/左/右/升/降。
...
class HelipadLandingNode(Node):
    ...
    def _output(self, frame):
        ...
        if hud_key != self._hud_key or self._hud_bar is None:
            ...
            put_cn_lines(bar, lines, origin=(10, 4), size=20)
            self._hud_bar = bar
            self._hud_key = hud_key
        vis[0:bar_h] = np.maximum(vis[0:bar_h], self._hud_bar)
        if extra:
            put_cn_lines(vis, extra, size=18)
        self.out.output(vis)
