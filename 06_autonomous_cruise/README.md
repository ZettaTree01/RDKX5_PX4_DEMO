...
解锁跟 05/07/08 一样走 `_common/offboard_manager.py`（强制解锁）：先爬升拉转速，
没航点时悬着保持转速，巡航时再按航点加速，最高 **300 r/min**。必须拆桨。

## 跑完这节能确认

- 任务怎么串起来：起飞 → 巡航 → 到点拍照 → 降落。
- 本节点只管航点，解锁 / 设定点 / 降落交给 OFFBOARD 管理器（含 `AUTO.LAND` 请求）。
- 拍照和航点可以绑在同一条任务里，后面换作业也好改。
- 飞的时候看巡航画面，能确认相机和当前航点进度。
...
> 必须拆桨。解锁后电机会先拉高再落到悬停转速，进入航点后会加速，最高 **300 r/min**。
