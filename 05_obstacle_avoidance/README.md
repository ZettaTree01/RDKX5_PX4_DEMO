...
障碍在画面偏左则向右让、偏下则上升。画面左上角显示相对高度与阶段；
触发避障时显示位移方向（前 / 后 / 左 / 右 / 上升 / 下降）并画箭头。

室内没 GPS 时 launch **默认带上台架位姿模拟**（`bench:=true`），不然飞控不让解锁。
台架会强制解锁；先爬升拉转速，没障碍时悬着保持转速，靠近障碍再按距离加速。必须拆桨。
...
画面左上角显示**相对开机高度**（室内气压计常漂到几十米，不再直接显示飞控绝对值）
与阶段（`等待解锁` / `起飞中` / 避障位移）。
触发避障时第二行显示位移方向，例如 `位移 后、左、上升`，右侧与底部有对应箭头。
纯 SSH 快照：`/tmp/avoid_snapshot.jpg`。
